...
class ExistingCountries(pycountry.db.Database):
    ...
    def wikipedia_redirects(self, aggressively_normalize=False):
        wik_redirs = dict()
        with open(
            os.path.join(LEXRES_DIR, "existingcountries_wikipedia_redirects.tab")) as f:
            for line in f:
    ...
    def search_fuzzy(self, query: str, shortcircuit=False):
        ...
        if (
            self.__class__ == ExistingCountries
        ):  # assuming data for historic countries not generated.
            ...
            elif normalize(query) in self.wikipedia_redirects(
                aggressively_normalize=True
            ):
                countrycode = self.wikipedia_redirects(aggressively_normalize=True)[
                    normalize(query)
                ]
                if shortcircuit:
                    return [self.lookup(countrycode)]
...
class Subdivision(pycountry.db.Data):
    def __init__(self, **kw):
        ...
        if self.parent_code is not None:
            self.parent_code = "{}-{}".format(self.country_code, self.parent_code)
...
countries: list[Country] = ExistingCountries(
