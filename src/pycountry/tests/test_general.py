...
def test_country_fuzzy_search():
    ...
    # before Australia.
    results = pycountry.countries.search_fuzzy("Wales")
...
def test_subdivisions_have_subdivision_as_parent():
    ...
    assert al_bu.code == "AL-BU"
    assert al_bu.name == "Bulqiz\xeb"
    assert al_bu.type == "District"
    assert al_bu.parent_code == "AL-09"
    assert al_bu.parent is pycountry.subdivisions.get(code="AL-09")
    assert al_bu.parent.name == "Dib\xebr"
